...
void Plateau::deplacer(int x_act, int y_act, int x_new, int y_new)
{
    if (mouvPossible(x_act, y_act, x_new, y_new))
    {
        ...
        m_plateau[x_act][y_act] = nullptr; 

        if (m_plateau[x_new][y_new]->m_dame == 0 && m_plateau[x_new][y_new]->m_couleur == "BLANC" && x_new == 0)
        {
            m_plateau[x_new][y_new]->m_dame = 1;
        }
        if (m_plateau[x_new][y_new]->m_dame == 0 && m_plateau[x_new][y_new]->m_couleur == "NOIR" && x_new == 9)
        {
            m_plateau[x_new][y_new]->m_dame = 1;
        }
    }
...
bool Plateau::mouvPossible(int x_act, int y_act, int x_new, int y_new)
{
    ...
    int dx = x_act - x_new;
    int dy = y_act - y_new;
    int x_int = (x_act + x_new) / 2; //Pour la prise des Pieces
    ...
    if (pieceA->m_dame)
    {
        ...
        //BLANC AVEC PRISE
            ...
            {
                if (prisePossibleDame(x_act, y_act, x_new, y_new, "BLANC"))
                {
                    prendreDame(x_act, y_act, x_new, y_new);
                    return true;
                }
                else { return false; }
            }
            else if (pieceA->m_couleur == "NOIR")
            {
                if (prisePossibleDame(x_act, y_act, x_new, y_new, "NOIR"))
                {
                    prendreDame(x_act, y_act, x_new, y_new);
                    return true;
                }
    ...
    else if (!pieceA->m_dame && couleurA == "BLANC")
    {
        ...
        {
            if (prisePossiblePiece(x_int, y_int, "BLANC"))
            {
                prendrePiece(x_int, y_int);
                return true;
            }
            else { return false; }
    ...
    else if (!pieceA->m_dame && couleurA == "NOIR")
    {
        ...
        {
            if (prisePossiblePiece(x_int, y_int, "NOIR"))
            {
                prendrePiece(x_int, y_int);
                return true;
            }
...
bool Plateau::prisePossiblePiece(int x_int, int y_int, string couleurPiece)
{
...
bool Plateau::prisePossibleDame(int x_act, int y_act, int x_new, int y_new, std::string couleurPiece)
{
    int dx = x_act - x_new;
    int dy = y_act - y_new;
    int x_int = (x_act + x_new) / 2; 
    int y_int = (y_act + y_new) / 2;

    if (dx == 2 && dy == 2) //Dame se comporte comme un Piece, elle prend en haut à droite
    {
        if (prisePossiblePiece(x_int, y_int, couleurPiece))
        {
            //Prendre 
            return true;
        }
    ...
    {
        if (diagLibre(x_act, y_act, x_new + 1, y_new + 1) && m_plateau[x_new+1][y_new+1]!=nullptr)
        {
            if (m_plateau[x_new + 1][y_new + 1]->m_couleur != couleurPiece)
            {
                return true;
            }
            else { return false; } //Piece de la meme couleur
        }
        else {
            return false;
        }
    ...
    else if (dx == 2 && dy == -2) //Dame se comp comme un Piece, prend haut gauche
    {
        if (prisePossiblePiece(x_int, y_int, couleurPiece))
        {
            //Prendre 
    ...
    else if (dx == -2 && dy == -2) //Deplacement en bas à droite
    {
        if (prisePossiblePiece(x_int, y_int, couleurPiece))
        {
            //Prendre 
...
void Plateau::prendrePiece(int x_int, int y_int) //Prend une position en paramètre, delete la piece associé, et met la case en nullptr
{
    if (m_plateau[x_int][y_int] != nullptr)
    {
        ...
        cout << "Prise impossible car pas de pièces présentes" << endl;
    }
}

void Plateau::prendreDame(int x_act, int y_act, int x_new, int y_new) {
    int dx = x_act - x_new;
    int dy = y_act - y_new;

    if (dx > 0 && dy > 0) {
        delete m_plateau[x_new + 1][y_new + 1];
        m_plateau[x_new + 1][y_new + 1] = nullptr;
    }
    else if (dx > 0 && dy < 0) {
        delete m_plateau[x_new + 1][y_new - 1];
        m_plateau[x_new + 1][y_new - 1] = nullptr;
...
vector <std::vector <Piece*>> Plateau::getPlateau() const {
    return m_plateau;
}